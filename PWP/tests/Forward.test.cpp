...
TEST_CASE("Forward O(h)") {
    ...
    SUBCASE("1a derivada") {
        Derivate forward(MethodType::Forward, DerivativeOrder::First, ErrorOrder::O_h);
        CHECK(forward.calculate(linear, xValue, hValue) == doctest::Approx(3.0).epsilon(hValue));
    }

    SUBCASE("2a derivada") {
        Derivate forward(MethodType::Forward, DerivativeOrder::Second, ErrorOrder::O_h);
        CHECK(forward.calculate(quadratic, xValue, hValue) ==
        doctest::Approx(2.0).epsilon(hValue));
    }

    SUBCASE("3a derivada") {
        Derivate forward(MethodType::Forward, DerivativeOrder::Third, ErrorOrder::O_h);
        CHECK(forward.calculate(cubic, xValue, hValue_d3) ==
              doctest::Approx(6.0).epsilon(hValue_d3));
    }

    SUBCASE("4a derivada") {
        Derivate forward(MethodType::Forward, DerivativeOrder::Fourth, ErrorOrder::O_h);
...
TEST_CASE("Forward O(h2)") {
    ...
    SUBCASE("1a derivada") {
        Derivate forward(MethodType::Forward, DerivativeOrder::First, ErrorOrder::O_h2);
...
TEST_CASE("Forward O(h2) mais preciso que O(h)") {
    const double xValue = 1.234;
    const double hValue = 1e-3;
    Derivate forwardOh(MethodType::Forward, DerivativeOrder::First, ErrorOrder::O_h);
    Derivate forwardOh2(MethodType::Forward, DerivativeOrder::First, ErrorOrder::O_h2);
