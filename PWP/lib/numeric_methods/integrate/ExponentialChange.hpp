...
double applyQ(const Stencil& s, Fn f, double a, double b) {
    double h   = b - a;
    double sum = 0.0;
...
template <typename Fn>
double adaptiveIntegrate(const Stencil& s, Fn f, double a, double b,
                         double tol = 1e-6, int max_iter = 50) {
    int    N    = 1;
    double prev = applyQ(s, f, a, b);
    for (int iter = 0; iter < max_iter; ++iter) {
        N *= 2;
        double dx   = (b - a) / N;
        double curr = 0.0;
        for (int i = 0; i < N; ++i)
            curr += applyQ(s, f, a + i * dx, a + (i + 1) * dx);
...
template <typename Fn>
double simpleExp(Fn f, double a, double b, const Stencil& s,
    ...
    return detail::adaptiveIntegrate(s, g, -c, c, tol);
}
...
template <typename Fn>
double doubleExp(Fn f, double a, double b, const Stencil& s,
